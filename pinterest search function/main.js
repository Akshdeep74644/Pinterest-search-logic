...
function showdata() {
  ...
  arr.forEach(function (elem) {
    dataHtml += `<div class="box">
                <img class="cursor-pointer" src="${elem.image}" alt="${elem.name}">
                <div class="caption">${elem.name}</div>
                </div>`;
  });
  document.querySelector(".container").innerHTML = dataHtml;
}

function searchHandle() {
  var inputFeild = document.querySelector("#searchinput");
  ...
  inputFeild.addEventListener("input", () => {
      ...
      elem.name.toLowerCase().startsWith(inputFeild.value)
      );
      var searchitem = "";
      filterArr.forEach((obj) => {
      searchitem += `<div class="res flex px-8 py-3">
            <i class="ri-search-line font-semibold mr-5"></i>
            <h3 class="font-semibold">${obj.name}</h3>
        </div>`;
    });
    document.querySelector(".searchdata").style.display = "block";
    document.querySelector(".searchdata").innerHTML = searchitem;
  });
